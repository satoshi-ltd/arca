...
Updated 2026-09-10. **v0.3.5 · Phase 1: functional alpha, stabilization in progress. Not a qualified public release.**
...
- Current checkout: v0.3.5 with the September 10 synchronization-integrity fixes below, implemented and tested locally. These changes have not been deployed to Casa or installed clients. Update hub and replicas together for the `pathTransitions` capability; older clients receive an upgrade error. Expo native builds remain manual and outside the release pipeline.
...
Validation: full local CI command passes **207 tests with one platform skip (208 total)**. New regression scenarios include both origins and sync modes, repeated case renames, excluded-content preservation, pending revision failure, timed pause restart, corrupt upload repair, healthy-folder progress, stale mobile deletion and relocation failure. The desktop web bundle and Android/iOS JavaScript exports also pass. Real Windows/Linux execution, native phone/Fold behavior, physical durability and deployment remain qualification boundaries.
