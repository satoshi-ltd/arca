...
Updated 2026-09-12. **v0.4.1 · Phase 1: functional alpha, stabilization in progress. Not a qualified public release.**
...
The card uses the concise copy “Deletes all local folders and resets Arca on this device. Hub files and other machines are kept.” Confirmation explicitly includes unsynced changes and irreversible deletion; desktop lists the saved folder paths and configured full-backup path. The replica's registered local folders are permanently deleted on desktop/server too; the designer reference's file-retention text is superseded by the user's explicit correction. Unrelated files in the default root, previously exported files and copies no longer registered to Arca are outside this operation. Hub content/history and other machines are unchanged; only this machine's registration/reports/credentials are removed there.

Local administrator `POST /v1/destroy-replica {confirmed:true}` interrupts sync, waits for active work and removes the hub registration before deleting local folders, configured full backup, transfer objects, indexes, queues and recovery checkpoints. While connected, an unreachable hub prevents deletion and allows retry. Already disconnected/revoked replicas can clean up without pairing again. A durable local deletion journal permits retry/restart after partial cleanup; changed directory identities or symlink substitutions are rejected rather than deleting a replacement directory. Pending destruction blocks normal mutations and sync. Successful cleanup rotates node identity and the local admin credential, clears browser sessions and marks `needsSetup`. Installation/listener configuration and the default root suggestion remain so the daemon stays reachable. Authenticated `POST /v1/setup` accepts a new name, role and root only in that first-run state; the native bridge reuses it instead of attempting to initialize an existing daemon again. Server web sessions require a fresh local web access code after destruction.
