...
Updated 2026-09-14. **v0.4.4 · Phase 1: functional alpha, stabilization in progress. Not a qualified public release.**
...
| Nonportable names (SYNC-13)                    | Stop the affected inventory safely and identify the offending path and required NFC/portable rename. Do not skip it and mistake an incomplete inventory for deletion.                                                                                                                                                                                                           |
...
The hub records optional validated phone capture timestamps with accepted proposals. Existing images read DateTimeOriginal, CreateDate or ModifyDate from bounded EXIF metadata. Fallbacks use recognized dated filenames, the original phone year/month path (month precision only), then the first accepted revision date, identified as date added rather than capture date. Phone timestamps use UTC days; offset-free EXIF uses the camera's recorded date. Metadata is content-addressed in `gallery_metadata`. Initial indexing advances in batches of 64 and waits for the initial index to settle before presenting ordered pages.
