...
Updated 2026-09-08. **v0.3.0 · Phase 1: functional alpha, stabilization in progress. Not a qualified public release.**
...
- Current work (September 8): desktop v0.2.3 pipeline run `34125966418` completed successfully, including Docker and all desktop packages. Mobile and supporting daemon/UI changes are uncommitted. Casa received bounded downloads, LAN access and durable conflict resolution; the Mac received the conflict-resolution daemon update. Later UI changes are local unless explicitly recorded as deployed. The newest file-detail and responsive work is verified through Metro on the existing Android emulator, not through a new native binary.
- Current task: consolidate current documentation and prepare a private Casa-only Docker update command. Preparing/checking the helper does not deploy this checkout. Current Android is paired over LAN after the explicitly requested app reset; old multi-folder test selections below are historical. Preserve live files, selections, backup and pause state. Do not restart Metro or build native binaries as incidental work.
...
Confirmed during review: IncomingShare's Cancel deleted the pending inbox despite copy promising preservation. Corrected locally: closing keeps pending files; explicit Discard shared files requires confirmation. All app-owned Sheet headers now share heading typography and an icon-only Close action, independent of content type; the header stays outside the scroll area. Native system alerts remain system UI.
...
| P2-MOBILE    | Implemented locally; qualification open      | Expo iOS/Android replica with persistent files, explicit import, full backup and OS-aware background work. Physical-device qualification and distribution configuration remain open.                                                                                                                                                                                                        |
...
Automatic working sync now uses a persistent SQLite dirty-path queue and a persistent remote cursor per folder. This replaces the former 3,000ms full-cycle timer; legacy `config.interval` values no longer set the automatic schedule.
...
- `GET /v1/changes?volume=ID&after=REV[&through=REV]` returns up to 500 current file/tombstone rows changed after the cursor, with a fixed upper revision bound and continuation. Catalog capability `changes:true` advertises support. Changes arriving during pagination are fetched on the next check. The cursor advances only after the folder's received work is successfully applied; invalid cursors and policy changes request reconciliation. Legacy hubs and backup nodes retain snapshot compatibility. Full backup remains separate from working synchronization.
...
- Backup is an optional capability of a replica, independent of selected working folders. It stores every hub folder and historical revision in a separate dedicated directory while normal bidirectional sync continues. Its backup copy never publishes edits. Dedicated legacy backup nodes remain compatible.
...
Removal uses the compatible hub-admin `/v1/revoke {id}` endpoint, now deleting the device row (name, credential hash, last address/contact), machine report, backup acknowledgement, snapshot leases/rows and partial uploads owned by that credential. Startup also purges legacy rows with `revoked=1` and their related data. A missing credential remains unauthorized; no denylist is needed. Queued work rechecks authorization when it starts, and POST requests recheck after reading the body, so a request authenticated before removal cannot recreate reports or uploads afterwards. Already-delivered bytes cannot be recalled.
...
- Settings → Hub backup on connected replicas: enable/disable, dedicated path, last completed backup, folder/revision counts and separate error reporting. Disabling retains the backup. Recovery uses its `state` subdirectory to reconstruct a new hub offline.
...
User decision (2026-09-07): mobile targets functional parity with a desktop replica and can NEVER act as a hub. Exclude hub creation/administration, pairing-code issuance and hub promotion/replacement. Preserve replica capabilities: explicit pairing/disconnect/reconnect, whole-folder selection, bidirectional synchronization with conflict preservation, pause/resume, local browsing/open/share/import, history/restore, and optional full hub backup. Platform filesystem and background-execution limits change how these capabilities work; they do not silently remove them from the scope. Independent full backup is implemented in portable-backup.js, requires explicit enablement, and uses storage checks and resumable verified transfers. Physical-device backup/recovery qualification remains open.
...
| Backup             | Linked replica/legacy backup/admin `GET /v1/archive` includes historical revisions                                                                                                |
...
| Machine management | Hub admin `/v1/devices {name,role}`, `/v1/revoke {id}`; replica/backup admin `/v1/connect {url,token,privateNetwork?}`                                                            |
...
`--check` reads SSH/Compose/status only. Normal invocation runs the Node test suite, snapshots only `packages`, `apps/desktop/src`, `package.json` and `deploy/Dockerfile`, uploads to a temporary remote build context, builds the image and recreates only Arca with a 120-second graceful stop allowance. Uncommitted source is included. It neither pulls a published image nor commits/pushes. A remote lock excludes simultaneous runs of this helper.
...
The previously running image is retained as `arca-pilot:previous`. Image rollback is not database rollback; inspect compatibility first. If appropriate, run:
...
Implemented locally, uncommitted: Expo Sharing receiving plugin configuration for Android SEND/SEND_MULTIPLE and iOS Share Extension; persistent incoming-file inbox; explicit destination restricted to shares already selected on this device and their subfolders; selection rechecked before import; local import with offline/pause queue behavior and existing conflict preservation. The inbox is separate from synchronized content and survives app restarts until Save or explicit confirmed Discard shared files. Closing the destination sheet preserves the inbox. Invalid paths, non-file payloads, missing/incomplete sources and ambiguous duplicate resolved URIs are rejected. Partial staging failure rolls back only that batch. The destination sheet stays mounted across view navigation.
...
Web alone shows current-browser Sign out and Sign out all browsers. Desktop alone shows launch at login and native notifications. Settings sections are always expanded, including Network, Service and Recovery; do not use disclosure toggles or accordions for these sections. Legacy backup machines stay compatible and do not offer upstream restore or local unlink.
...
### Receive files from other apps — September 8

The system Share destination is Arca; the destination sheet is titled Save file (one item) or Save files (several). Saving chooses a local synchronized destination; it is not presented as a direct save to the hub. Copy incoming files to a persistent private inbox before asking for a destination. Closing with X preserves the incoming batch. Explicit confirmed Discard shared files removes only private inbox copies; source files stay untouched. An interrupted app session retains pending files and restores the sheet on restart. Pair first if disconnected from any hub. Never upload before Save. List only shares already selected for sync on this device. Browse their locally known subdirectories or enter a relative subfolder such as workouts/2026. Incoming sharing never selects or downloads another share. With no selected folders, explain that the user must select one in Folders first and retain the inbox. Recheck selection at Save. Existing selected folders accept local saves while offline or paused; normal sync uploads later. Existing differing content is preserved by the normal import conflict behavior. Discard never removes source files or synchronized files.

Use the shared Sheet, FolderRow (icon, left-aligned name, metadata and chevron), Button, Field and Card primitives. Show incoming files in a grouped summary and the chosen path in a folder card. Save here is the single primary action. The header closes with X and preserves pending files; explicit Discard shared files is a separate confirmed action. Use 12-dp section spacing, one compact file-summary row and contiguous folder rows instead of individual spaced cards. Reject links/text, unsafe paths, incomplete files and duplicate resolved temporary filenames with a recoverable message. Native receipt supports up to 20 files per batch; cap the pending inbox at 100. Do not claim every source app or iOS is qualified: Expo 55 incoming sharing is experimental, particularly its iOS extension. A native build is required to register the system target.
...
Mobile's app-owned storage differs from desktop's user-owned destinations. Stop syncing… confirms “Stop syncing and remove local files?” with a “Remove local files” action. It removes only the working directory on this device and its local registration/index, returning the share to On hub · Not selected. Hub files, history, other devices and the independent backup remain. Unreferenced transfer objects are reclaimed; objects referenced by other folders, pending operations, recovery journals or backup history remain. Pause and hub disconnect continue to preserve local files.

Before removal, stop the active sync and verify actual file hashes against accepted revisions. Block queued uploads, unfinished replacements, new/modified files (including excluded files) and local deletions awaiting sync. Explain that the user must sync or export unsaved files first. A durable removal marker allows retry after interrupted filesystem cleanup and prevents automatic redownload. No retained unselected Local copy state is offered. Failed removal retains its durable marker and actionable error; a missing hub share preserves the local index. No legacy data is silently cleaned up. Verified with isolated replica tests; no live user folder was removed during implementation.

#### Compact machine roster

Desktop, server web and mobile keep Hub connection separate, then one Machines list containing the current machine and other registered machines (including the hub when administering it). Do not split This machine and Other machines into sections. Each item has two content lines: name, Hub/Replica role and This machine tag; then OS and the known network/address. Omit loopback, local daemon ports, app version and Last contact text from registered rows. Tailscale is shown when discovery confirms the peer; an observed IP alone does not prove its transport. Discovered, unpaired machines remain separate and retain compatibility/version information. Desktop/Fold may show a trailing status badge; phone uses the two-line identity/address row with the status retained in its accessibility label. Backup information stays in its own section. Implemented locally; Casa web requires a deployment to receive this UI.
...
Phone Hub connection is title/address plus a labelled disconnect icon with confirmation, omitting the repeated sync explanation. Phone Hub backup is a compact navigable two-line row. Fold keeps labelled right-hand buttons, machine status/totals on the right, monospaced network details, and horizontal backup summary. Both retain the same information hierarchy and semantic colors. Visually reviewed Machines in phone and Fold on the existing Android emulator; this does not claim every screen or OS rasterization is pixel-identical.
...
Files, Recent and History open the same mobile file-detail component: summary, retained revisions and file location. Wide windows use the desktop two-column structure; phone stacks the sections. Available local files expose Open / share in the header and editing, copy-path and deletion in the shared overflow sheet. Navigation origin only determines the back destination; browsing retains its directory. There is no separate reduced file-action card for explorer entries. Loading, offline and failed history requests must not imply an available hub revision.
